...
will never give you an empty array (that's not a walk, that's standing 
still!).
*/

function isValidWalk(walk) {
    let streets = walk.length;

    if (streets != 10) {
        return false;
    }

    if (walk.filter((steps) => steps == 'n').length == walk.filter((steps) => steps == 's').length 
        && walk.filter((steps) => steps == 'e').length == walk.filter((steps) => steps == 'w').length) {
        return true;
    }else{
        return false;
    }
